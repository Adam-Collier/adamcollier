...
const Resource = ({ id, link, title, summary, description, user }: ResourceProps) => (
  <div key={id} className="grid grid-cols-[140px,1fr] md:grid-cols-[200px,1fr]">
    <div>
      ...
      {user &&
        <Link to={`/admin/resources/edit/${id}`} className="hover:underline"><i className="text-sm flex items-center gap-1 hover:underline mt-0.5">Edit <span className="inline-block i-ri:arrow-right-line" /></i></Link>
      }
    </div>
