...
export const loader: LoaderFunction = async () => {
  ...
  if (!process.env.NOTION_INTERNET_RADIO_STATIONS) {
    throw new Response("notion internet radio station id needed", { status: 500 })
  }
...
const Music = () => {
  ...
  return (
    <section className="flex h-full">
      ...
      <div className="p-4 basis-5/6 space-y-4" style={{ backgroundImage: "linear-gradient(to bottom, hsl(0, 0%, 39%) 0%, hsl(0, 0%, 35.62%) 2.5%, hsl(0, 0%, 32.33%) 4.4%, hsl(0, 0%, 29.13%) 6.2%, hsl(0, 0%, 26.05%) 7.9%, hsl(0, 0%, 23.06%) 9.9%, hsl(0, 0%, 20.18%) 12.4%, hsl(0, 0%, 17.42%) 15.7%, hsl(0, 0%, 14.77%) 20%, hsl(0, 0%, 12.24%) 25.6%, hsl(0, 0%, 9.84%) 32.7%, hsl(0, 0%, 7.58%) 41.6%, hsl(0, 0%, 5.45%) 52.5%, hsl(0, 0%, 3.47%) 65.7%, hsl(0, 0%, 1.65%) 81.5%, hsl(0, 0%, 0%) 100%" }}>
        <div className="h-[24px] flex justify-between items-center">
          <SpotifyLogo className="h-full w-auto" />
          {user && <Link to="/admin/music/new/spotify" className="text-white text-xs">Add an Album</Link>}
        </div>

        <div className="grid grid-cols-8 gap-2">
          {spotifyAlbums.map(({ image, link, rating }: { image: string, link: string, rating: number }, index: string) => (
      ...
      <div className="p-4 bg-gradient-to-t from-[#f30] to-[#f70] space-y-4 flex-grow">
        <div className="h-[24px] flex items-center justify-between">
          ...
          {user && <Link to="/admin/music/new/soundcloud" className="text-white text-xs">Add a mix</Link>}
        </div>
        <div className="flex flex-col gap-2">
          {soundcloudMixes.map(({ artist, title, link, image }: { artist: string, title: string, link: string, image: string }, index: number) => {
            return (
              <a className="block w-full relative flex items-start text-white bg-gradient-to-r to-white/3 hover:to-white/10 from-transparent rounded overflow-hidden" href={link} target="_blank" rel="noreferrer noopener" key={index}>
      ...
      <div className="bg-black p-2 flex flex-col justify-between items-center bg-gradient-to-t from-orange-300 to-orange-100">
        <div className="flex flex-col gap-2">
          {radioStations.map(({ name, link, image }: { name: string, link: string, image: string }) => (
            <a href={link} className="relative block rounded w-10 h-10 border border-gray-800" target="_blank" rel="noreferrer noopener">
