  ...
  if (url.startsWith("https://open.spotify.com")) {
    ...
    walk(ast, {
      enter: (node, parent) => {
        ...
        if (node.type === SyntaxKind.Tag && node.name === "meta" &&
          node.attributes[0].value?.value.match(/^(og:title|og:description|og:image)$/)) {
    ...
    walk(ast, {
      enter: (node) => {
        ...
        if (node.type === SyntaxKind.Tag && node.name === "link" && node.attributeMap.rel.value.value === "apple-touch-icon") {
