						...
						return (
							<li key={index}>
								<Link to={`/resources/${toSlug(name)}`} className="group block">
									<p className="font-semibold hover:underline group-hover:underline">{name}</p>
									<small className="text-gray-400 text-xs"><i>Updated: {new Date(updatedAt).toLocaleDateString("en-US", dateOptions)}</i></small>
								</Link>
							</li>
						)
					}
